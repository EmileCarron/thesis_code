    ...
    def postprocess_detections(self, head_outputs, anchors, image_shapes):
        ...
        for index in range(num_images):
            ...
            detections.append({
                ...
                'embedding': image_embedding[keep],
            })
...
class RetinaNetLightning(pl.LightningModule):
    ...
    def teacher(self, args):
        ...
        teacher_model = teacher.load_from_checkpoint(checkpoint_path=args.checkpoint2, args=args)
...
class Predictions(pl.LightningModule):
    ...
    def predicitions(self, args):
        ...
        for y in range(len(list_of_image)):
            print(list_of_image[y])
            imagein = cv2.imread(list_of_path[y])
            self.model.eval()
            imagelist = []
            ...
            torch.save(predictions, args.save + 'tensor_' + list_of_image[y] +'.pt')
            ...
            for idx in range(len(predictions[0]['boxes'])):
                if predictions[0]['scores'][idx] > 0.3:
                    ...
                    height = predictions[0]['boxes'][idx][3]-predictions[0]['boxes'][idx][1]
                    width = predictions[0]['boxes'][idx][2]-predictions[0]['boxes'][idx][0]
                    if height < 7:
                        height = 7
                    if width < 7:
                        width = 7
                    imagec = torchvision.transforms.functional.crop(image, predictions[0]['boxes'][idx][1].int(), predictions[0]['boxes'][idx][0].int(), height.int(), width.int())
                    imagec = torchvision.transforms.functional.to_pil_image(imagec)
                    imagec = cv2.cvtColor(np.asarray(imagec),cv2.COLOR_RGB2BGR)
                    imagec = cv2.cvtColor(np.asarray(imagec),cv2.COLOR_RGB2BGR)
                    cv2.imwrite(args.save + "/crop"+ list_of_image[y]+ "/crop" + str(idx) +"-"+str((predictions[0]['labels'][idx]).data)+ ".png", imagec)
            cv2.imwrite(args.save + "/output_emb_" + list_of_image[y] +".png", imagein)
